...
2. Before staging anything, scan the diff for secrets (API keys, `.env` values, service role keys) — see `security.md`. If found, unstage and fix before committing.
...
fix(scripts): correct LLM template interpolation
fix(db): add missing RLS policy on model_cards
docs: update schema documentation for curator_flags
chore: bump next.js to 15.x
test: add verification script for model fact accuracy
