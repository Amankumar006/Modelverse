...
// Pure display formatters live in `model-format.ts` (no DB imports) so that
// client components can use them without bundling the Supabase client.
// Re-exported here for server-side / back-compat callers.
export { formatParameters, getModalities } from "./model-format";
