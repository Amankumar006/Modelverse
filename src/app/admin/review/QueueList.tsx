...
export default function QueueList({ models }: { models: Model[] }) {
  ...
  return (
    <div className="space-y-6">
      <div className="flex justify-between items-center bg-daylight-card p-4 rounded-xl shadow-sm border border-daylight-muted/20">
        <div className="text-sm text-daylight-text">
          <span className="font-bold">{models.length}</span> models
        </div>
        <button 
          onClick={handleDismiss}
          disabled={selectedSlugs.size === 0 || isDismissing}
          className="px-4 py-2 bg-daylight-accent/10 text-daylight-accent font-medium rounded-lg hover:bg-daylight-accent/20 disabled:opacity-50 disabled:cursor-not-allowed transition-colors"
        >
      ...
      <div className="space-y-4">
        ...
        {models.length === 0 && (
          <div className="p-12 text-center text-daylight-muted bg-daylight-card rounded-xl border border-daylight-muted/20">
            No pending models to review. You&apos;re all caught up!
          </div>
        )}
      </div>
    </div>
