...
4. If the release includes a Supabase migration, it has already been applied and verified against prod (follow the `supabase-migrations` skill) — don't bundle an unapplied migration with code that depends on it.
...
2. Push, then watch the Vercel deployment until it's marked Ready — the Vercel MCP server is connected, so deployment status and logs can be checked through it.
