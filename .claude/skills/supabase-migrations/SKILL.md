...
1. Run the Supabase MCP's `get_advisors` for both `security` and `performance` types and resolve or note anything new the migration surfaced.
2. Regenerate TypeScript types if the project uses generated Supabase types, and check `src/` still compiles against them.
3. Update any Zod schemas in `data/` or `scripts/` that mirror the changed table shape — these validate the ingestion pipeline's writes and will silently reject or miscast data if they drift from the real schema.
